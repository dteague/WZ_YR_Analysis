...
if '__main__' == __name__:    
    ...
    lhe = EventFile(sys.argv[1])
    filenumbers = int(sys.argv[2])
    output_area = sys.argv[3]

    output_list = []
    for i in xrange(filenumbers):

        output_list.append(        open(output_area+'/split_files/output_events'+str(i)+'.lhe', 'w'))
        output_list[i].write(lhe.banner)
    #write the banner to the output file

    # Loop over all events
    for count, event in enumerate(lhe):
        #write this modify event
        output_list[count%filenumbers].write(str(event))

    for files in output_list:
        files.write("</LesHouchesEvents>")
        files.close()
